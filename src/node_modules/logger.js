...
export default class Logger extends Writable {
			...
			if (secure) {
				connection = tls.connect.apply(tls, arguments, () => {
					if (!connection.authorized) {
						DEBUG.log("Secure connection -TLS- is not authorized.");
	...
	@enumerable
	closeConnection() {
		DEBUG.log("Closing retry mechanism along with its connection.");
	...
	async [$getConnection]() {
		...
		if (this[$connection]) {
			...
			DEBUG.log("No connection exists. Creating a new one.");
	...
	getConnectionPromise() {
		...
		this[$connection] = new Promise((resolve, reject) => {
			...
			this[$reconnection].on('reconnect', function (n, delay) {
				if (n > 0)
					DEBUG.log("Trying to reconnect. Times: " + n + " , previous delay: " + delay);
			});
			...
			this[$reconnection].on('error', function (err) {
				DEBUG.log("Error occurred during connection: " + err);
			});
		...
		this[$reconnection].on('connection timeout', () => {
			...
			DEBUG.log("Socket was inactive for a period of time. Destroying.");
